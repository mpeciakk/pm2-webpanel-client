...
const Host = ({ name }) => {
    ...
    return (
        processes && stats && <Collapsible
            title={
                <div className="flex w-full font-medium text-xl">
                    <h6>{name}</h6>

                    <div className="self-end ml-auto mr-5 flex items-center justify-around w-2/6">
                        <FaMemory /> {Math.floor(stats.memory / 1024 / 1024)}MB/{Math.floor(stats.maxMemory / 1024 / 1024)}MB <BsCpu /> {stats.cpu}% <FiHardDrive /> {stats.disk}%
                    </div>
                </div>
            }
            content={
                        ...
                        <tbody>
                            {processes &&
                                processes.map((process) => (
                                    <tr key={process.id} className={`h-10 border-b ${process.status === 'stopped' && 'bg-red-600'}`}>
                                        <th>{process.id}</th>
                                        <td>{process.name}</td>
